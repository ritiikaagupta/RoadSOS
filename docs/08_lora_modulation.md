...
LoRa uses Chirp Spread Spectrum (CSS) modulation. A chirp is a
signal whose instantaneous frequency changes continuously with time.

## 2. Upchirp

An upchirp is a chirp whose frequency increases with time.

The frequency sweeps across the selected LoRa bandwidth.

## 3. Downchirp

A downchirp is a chirp whose frequency decreases with time.

## 4. LoRa Parameters Used for Initial Simulation

- Bandwidth: 125 kHz
- Spreading Factor: 7
- Number of possible symbols: 128
- Bits per symbol: 7
- Symbol duration: 1.024 ms

## 5. Bits to Symbols

For SF = 7, the input bitstream is divided into groups of
7 bits.

Each 7-bit group produces one symbol value from 0 to 127.

## 6. Symbol to Chirp Mapping

A LoRa symbol is generated by applying a symbol-dependent
cyclic shift to a reference chirp.
...
Symbols
↓
Cyclic shift / chirp mapping
↓
LoRa modulated waveform

## 7. Initial FPGA Relevance

The Python implementation will be used as a reference model before
implementing the LoRa modulation algorithm using FPGA hardware.

The FPGA implementation will eventually require:
- Symbol generation
- Chirp generation
- Phase accumulation
- Timing control
- Sample generation
- Interface to the external LoRa transceiver
